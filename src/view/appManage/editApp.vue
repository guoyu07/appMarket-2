<template>
  <div id="addApp"> 
    <h1><span>当前位置 > </span><a href="#/index/appManage">应用管理</a><span> > </span><span class="title_active">{{$route.query.type=='0'?'版本升级':'编辑'}}</span></h1>
        ...
        <div class="form_wrap">
          ...
          <Form ref='formValidate' :model="appInfo" :label-width="110" :rules="ruleValidate">
            <FormItem label="应用名"  prop="appName">
               <Input v-model="appInfo.appName" placeholder="建议20字以内，不超过100个字。" disabled  style="width:400px"></Input>
            </FormItem>
...
// created------------------------------------------------------------------------------------
  created(){
    document.title="应用管理-" + this.$route.query.type=='0'?'版本升级':'编辑';
    ...
    queryDetail(){
      this.axios.get('/app/getDetail',{params:{
        ...
        if(res && res.success=='1'){
          ...
          this.app.updateDate = data.updateDate
          this.app.logoPath = data.logoPath
          this.app.size = data.size
          this.app.apkPath = data.apkPath
          // appInfo
          this.appInfo.uploadList = data.captureUrls.split(";").map(item=>{
            return {url:item}
          })
          this.$refs.upload.fileList = this.appInfo.uploadList
          this.appInfo.appName = data.appName
          this.appInfo.tag = data.tag
          this.appInfo.type = data.type
          this.appInfo.classify = data.classify
          this.appInfo.summary = data.summary
          this.appInfo.introduce = data.introduce
          this.appInfo.featureDesc = data.featureDesc
          this.appInfo.authority = data.authority
          this.appInfo.ifChare = data.ifChare
          this.appInfo.hasAd = data.hasAd
          this.appInfo.supportLanguage = data.supportLanguage
          this.appInfo.iconUrl = data.iconUrl
          this.appInfo.captureUrls = data.captureUrls
